const STORAGE = {
  ...
  accessRoles: "meirahgroup.accessRoles",
  staffCategories: "meirahgroup.staffCategories",
  ...
  companyProfile: "meirahgroup.companyProfile",
};
...
const CURRENCY_RATES = {
  USD: 1,
  // Base currency: USD. Reference update: 2026-08-25.
  // EUR/USD: ECB reference rate (EUR 1 = USD 1.1662).
  // DOP/USD: Banco Central de la Republica Dominicana selling rate.
  EUR: 0.8572,
  DOP: 58.5044,
};
...
const BUSINESS_PHONE_DISPLAY = "+1 (849) 595-5951";
const BUSINESS_WHATSAPP = "18495955951";
const DEFAULT_COMPANY_PROFILE = {
  name: BUSINESS_NAME,
  description: "Firma inmobiliaria para compra, venta y renta de propiedades con asesoría cercana.",
  coverage: "Jarabacoa, La Vega y República Dominicana.",
  services: "Compra, venta, renta corta y larga, promoción de propiedades, reservas, visitas y asesoría de inversión.",
  address: "Av. Federico Basilis, Jarabacoa, La Vega, RD",
  hours: "Lunes a viernes 9 a.m. - 5 p.m. | Sábados citas previas.",
  phone: BUSINESS_PHONE_DISPLAY,
  email: "contacto@meirahgroup.com",
  languages: "Español, English y Português.",
  reservationPolicy: "Las reservas están sujetas a disponibilidad y confirmación del equipo.",
  appointmentPolicy: "Las citas dependen de la disponibilidad del agente seleccionado.",
  faqs: "¿Qué servicios ofrecen?|Compra, venta, renta corta y larga, reservas, visitas y asesoría inmobiliaria.\n¿En qué zonas trabajan?|Jarabacoa, La Vega y otras zonas de República Dominicana.\n¿Puedo vender o alquilar mi propiedad?|Sí. Evaluamos la propiedad y la representamos con estrategia de precio, promoción y seguimiento.",
  additionalInfo: "",
};
const PROPERTY_CODE_WIDTH = 3;
const PROPERTY_ROUTE_PREFIX = "/propiedades/";
const STAFF_PERMISSION_DEFINITIONS = [
  { key: "manageProperties", label: "Propiedades" },
  { key: "viewAppointments", label: "Ver citas" },
  { key: "manageAppointments", label: "Gestionar citas" },
  { key: "viewLeads", label: "Leads" },
  { key: "manageStaff", label: "Usuarios y roles" },
  { key: "manageSchedules", label: "Horarios del equipo" },
  { key: "ownAppointments", label: "Citas propias" },
  { key: "ownAvailability", label: "Horario propio" },
];
const DEFAULT_ACCESS_ROLES = [
  { id: "ceo", name: "CEO", color: "#b78943", permissions: { manageProperties: true, viewAppointments: true, manageAppointments: true, viewLeads: true, manageStaff: true, manageSchedules: true, ownAppointments: true, ownAvailability: true } },
  { id: "developer", name: "Developer", color: "#5b6fd8", permissions: { manageProperties: true, viewAppointments: true, manageAppointments: true, viewLeads: true, manageStaff: true, manageSchedules: true, ownAppointments: true, ownAvailability: true } },
  { id: "advisor", name: "Asesor", color: "#2d9474", permissions: { manageProperties: false, viewAppointments: true, manageAppointments: false, viewLeads: false, manageStaff: false, manageSchedules: false, ownAppointments: true, ownAvailability: true } },
  { id: "operations", name: "Operaciones", color: "#c46f4e", permissions: { manageProperties: false, viewAppointments: true, manageAppointments: true, viewLeads: true, manageStaff: false, manageSchedules: true, ownAppointments: false, ownAvailability: false } },
  { id: "content", name: "Contenido", color: "#8b67a8", permissions: { manageProperties: true, viewAppointments: false, manageAppointments: false, viewLeads: false, manageStaff: false, manageSchedules: false, ownAppointments: false, ownAvailability: false } },
];
const DEFAULT_STAFF_CATEGORIES = [
  { id: "direction", name: "Dirección", color: "#b78943" },
  { id: "technology", name: "Tecnología", color: "#5b6fd8" },
  { id: "commercial", name: "Comercial", color: "#2d9474" },
  { id: "operations", name: "Operaciones", color: "#c46f4e" },
  { id: "creative", name: "Creativo", color: "#8b67a8" },
];
let accessRoles = loadAccessRoles();
let staffCategories = loadStaffCategories();
const APPOINTMENT_STATUSES = ["pending", "confirmed", "cancelled", "completed"];
const ACTIVE_APPOINTMENT_STATUSES = ["pending", "confirmed"];
const CRM_STAGES = [
  { id: "new", label: "Nuevo", icon: "sparkles" },
  { id: "qualified", label: "Calificado", icon: "badge-check" },
  { id: "visit", label: "Visita", icon: "calendar-check" },
  { id: "offer", label: "Oferta", icon: "hand-coins" },
  { id: "closed", label: "Cierre", icon: "trophy" },
];
const CRM_PRIORITIES = ["high", "medium", "low"];
const WEEKDAY_KEYS = ["monday", "tuesday", "wednesday", "thursday", "friday", "saturday", "sunday"];
const STAFF_SEED_VERSION = "10-custom-access-profiles";
const DEFAULT_STAFF = [
  {
    id: "staff-diego-cueto",
    name: "Diego Cueto",
    username: "diego.cueto",
    password: "1234",
    role: "developer",
    categoryId: "technology",
    permissions: defaultStaffPermissions("developer"),
    active: true,
  ...
  {
    id: "staff-melissa-hawa",
    name: "Melissa Hawa",
    username: "melissa.hawa",
    password: "1234",
    role: "ceo",
    categoryId: "direction",
    permissions: defaultStaffPermissions("ceo"),
    active: true,
    acceptsAppointments: true,
    availabilityConfigured: true,
    phone: BUSINESS_WHATSAPP,
    email: "melissa@meirahgroup.com",
    avatar: "",
    availability: {
      slotDuration: 45,
      weekly: {
  ...
  },
  {
    ...
    permissions: defaultStaffPermissions("advisor"),
    active: true,
    acceptsAppointments: false,
    availabilityConfigured: false,
    phone: BUSINESS_WHATSAPP,
    email: "juan.perez@meirahgroup.com",
    avatar: "",
    availability: {
      slotDuration: 60,
      weekly: {
  ...
  },
  {
    ...
    availabilityConfigured: false,
    phone: BUSINESS_WHATSAPP,
    email: "laura.mendez@meirahgroup.com",
    avatar: "",
    availability: { slotDuration: 60, weekly: {}, blockedDates: [], blockedSlots: [] },
  },
  {
    id: "staff-carlos-santana",
    name: "Carlos Santana",
    ...
    availabilityConfigured: false,
    phone: BUSINESS_WHATSAPP,
    email: "carlos.santana@meirahgroup.com",
    avatar: "",
    availability: { slotDuration: 60, weekly: {}, blockedDates: [], blockedSlots: [] },
  },
  {
    id: "staff-sofia-rosario",
    name: "Sofía Rosario",
    username: "sofia.rosario",
    password: "demo123",
    role: "operations",
    categoryId: "operations",
    permissions: defaultStaffPermissions("operations"),
    active: true,
    acceptsAppointments: false,
    availabilityConfigured: false,
    phone: BUSINESS_WHATSAPP,
    email: "sofia.rosario@meirahgroup.com",
    avatar: "",
    availability: { slotDuration: 60, weekly: {}, blockedDates: [], blockedSlots: [] },
  },
  {
    id: "staff-mateo-guzman",
    name: "Mateo Guzmán",
    username: "mateo.guzman",
    password: "demo123",
    role: "content",
    categoryId: "creative",
    permissions: defaultStaffPermissions("content"),
    active: true,
    acceptsAppointments: false,
    availabilityConfigured: false,
    phone: BUSINESS_WHATSAPP,
    email: "mateo.guzman@meirahgroup.com",
    avatar: "",
    availability: { slotDuration: 60, weekly: {}, blockedDates: [], blockedSlots: [] },
  },
  {
    id: "staff-camila-reyes",
    name: "Camila Reyes",
    ...
    availabilityConfigured: false,
    phone: BUSINESS_WHATSAPP,
    email: "camila.reyes@meirahgroup.com",
    avatar: "",
    availability: { slotDuration: 60, weekly: {}, blockedDates: [], blockedSlots: [] },
  },
  {
    id: "staff-andres-pena",
    name: "Andrés Peña",
    username: "andres.pena",
    password: "demo123",
    role: "operations",
    categoryId: "operations",
    permissions: defaultStaffPermissions("operations"),
    ...
    availabilityConfigured: false,
    phone: BUSINESS_WHATSAPP,
    email: "andres.pena@meirahgroup.com",
    avatar: "",
    availability: { slotDuration: 60, weekly: {}, blockedDates: [], blockedSlots: [] },
  },
  {
    id: "staff-valentina-cruz",
    name: "Valentina Cruz",
    ...
    availabilityConfigured: false,
    phone: BUSINESS_WHATSAPP,
    email: "valentina.cruz@meirahgroup.com",
    avatar: "",
    availability: { slotDuration: 60, weekly: {}, blockedDates: [], blockedSlots: [] },
  },
];
...
const I18N = {
  es: {
    ...
    sortPriceAsc: "Menor a mayor precio",
    sortPriceDesc: "Mayor a menor precio",
    sortAreaAsc: "Menor a mayor m²",
    sortAreaDesc: "Mayor a menor m²",
    ...
    aboutTitle: "Espacios con propósito",
    aboutTextOne: "Acompañamos decisiones inmobiliarias claras, cercanas y enfocadas en tu patrimonio.",
    aboutTextTwo: "Claridad y seguimiento de principio a fin.",
    aboutTextThree: "Tu esencia. Tu espacio. Tu patrimonio.",
    visionEyebrow: "Visión",
    visionTitle: "Mirar a largo plazo",
    visionTextOne: "Ser una firma inmobiliaria confiable y moderna, reconocida por convertir decisiones patrimoniales en oportunidades duraderas.",
    visionTextTwo: "Crecemos junto a nuestros clientes, con relaciones cuidadas y visión de futuro.",
    missionEyebrow: "Misión",
    missionTitle: "Guiar con claridad",
    missionCopy: "Facilitamos cada compra, venta o renta con información clara, acompañamiento cercano y un proceso ordenado.",
    missionCopyTwo: "Buscamos que cada decisión se sienta segura y esté alineada con las metas de cada cliente.",
    valuesEyebrow: "Valores",
    valuesTitle: "Nuestros principios",
    valuesCopy: "Trabajamos con transparencia, cercanía y criterio profesional para que cada relación empiece y termine con confianza.",
    visionValueOne: "Confianza",
    visionValueTwo: "Estrategia",
    visionValueThree: "Patrimonio",
    valueOne: "Claridad",
    valueTwo: "Cercanía",
    valueThree: "Confianza",
    reviewsEyebrow: "Reseñas",
    reviewsTitle: "Referencias verificadas de clientes",
    ...
    backupTitle: "Respaldo local",
    backupCopy: "Guarda una copia de propiedades, reservas, citas, CRM y configuración de este navegador.",
    backupExport: "Exportar datos",
    backupImport: "Restaurar respaldo",
  ...
  en: {
    ...
    navOwners: "Sell/Rent",
    navContact: "Contact",
    heroSlogan: "Your Essence • Your Space • Your Wealth",
    ...
    sortPriceAsc: "Lowest to highest price",
    sortPriceDesc: "Highest to lowest price",
    sortAreaAsc: "Lowest to highest sqm",
    sortAreaDesc: "Highest to lowest sqm",
    sortRecent: "Recent",
    ...
    aboutTitle: "Spaces with purpose",
    aboutTextOne: "We guide clear, close, and value-focused real estate decisions.",
    aboutTextTwo: "Clear guidance from start to finish.",
    aboutTextThree: "Your essence. Your space. Your legacy.",
    visionEyebrow: "Vision",
    visionTitle: "Think long term",
    visionTextOne: "To be a trusted, modern real estate firm known for turning wealth decisions into lasting opportunities.",
    visionTextTwo: "We grow alongside our clients through thoughtful relationships and a forward-looking perspective.",
    missionEyebrow: "Mission",
    missionTitle: "Guide with clarity",
    missionCopy: "We make every purchase, sale, or rental easier with clear information, close guidance, and an organized process.",
    missionCopyTwo: "We want each decision to feel secure and aligned with every client's goals.",
    valuesEyebrow: "Values",
    valuesTitle: "Our principles",
    valuesCopy: "We work with transparency, closeness, and professional judgment so every relationship begins and ends with trust.",
    visionValueOne: "Trust",
    visionValueTwo: "Strategy",
    visionValueThree: "Legacy",
    valueOne: "Clarity",
    valueTwo: "Closeness",
    valueThree: "Trust",
    reviewsEyebrow: "Reviews",
    reviewsTitle: "Verified client references",
    ...
    backupCopy: "Save a copy of this browser's properties, bookings, appointments, CRM, and settings.",
...
I18N.pt = {
  ...
  navContact: "Contato",
  heroSlogan: "Sua Essência • Seu Espaço • Seu Patrimônio",
  heroEyebrow: "Jarabacoa como base, República Dominicana como alcance",
  heroCopy:
  ...
  financeTitle: "Calculadoras financeiras",
  ...
  sortPriceAsc: "Menor para maior preço",
  sortPriceDesc: "Maior para menor preço",
  sortAreaAsc: "Menor para maior m²",
  sortAreaDesc: "Maior para menor m²",
  sortRecent: "Recentes",
  ...
  aboutTitle: "Espaços com propósito",
  aboutTextOne: "Guiamos decisões imobiliárias claras, próximas e focadas no seu patrimônio.",
  aboutTextTwo: "Clareza e acompanhamento do início ao fim.",
  aboutTextThree: "Sua essência. Seu espaço. Seu patrimônio.",
  visionEyebrow: "Visão",
  visionTitle: "Pensar no longo prazo",
  visionTextOne: "Ser uma imobiliária confiável e moderna, reconhecida por transformar decisões patrimoniais em oportunidades duradouras.",
  visionTextTwo: "Crescemos junto aos nossos clientes, com relações cuidadas e visão de futuro.",
  missionEyebrow: "Missão",
  missionTitle: "Guiar com clareza",
  missionCopy: "Facilitamos cada compra, venda ou aluguel com informações claras, acompanhamento próximo e um processo organizado.",
  missionCopyTwo: "Buscamos que cada decisão seja segura e esteja alinhada aos objetivos de cada cliente.",
  valuesEyebrow: "Valores",
  valuesTitle: "Nossos princípios",
  valuesCopy: "Trabalhamos com transparência, proximidade e critério profissional para que cada relação comece e termine com confiança.",
  visionValueOne: "Confiança",
  visionValueTwo: "Estratégia",
  visionValueThree: "Patrimônio",
  valueOne: "Clareza",
  valueTwo: "Proximidade",
  valueThree: "Confiança",
...
// Portuguese starts from the Spanish dictionary above; these overrides keep every
// public and internal label in Portuguese instead of falling back to Spanish.
Object.assign(I18N.pt, {
  ...
  favoritesButton: "Favoritos",
  calculatorsTitle: "Calculadoras",
  refinanceTitle: "Calculadora de refinanciamento",
  refinanceCardCopy: "Compare sua parcela atual com uma nova taxa.",
  homePrice: "Preço do imóvel",
  downPayment: "Entrada",
  interestRate: "Taxa anual",
  loanYears: "Prazo em anos",
  autopayTitle: "Cobrança automática para pagamentos parcelados",
  autopayCardCopy: "Gere um calendário de pagamentos recorrentes.",
  ...
  monthlyDebt: "Dívidas mensais",
  affordDown: "Entrada disponível",
  monthlyRent: "Aluguel mensal",
  yearsInHome: "Anos no imóvel",
  clientName: "Nome do cliente",
  paymentAmount: "Valor mensal",
  paymentDay: "Dia de cobrança",
  paymentMonths: "Quantidade de pagamentos",
  scheduleAutopay: "Gerar plano",
  reservationEyebrow: "Aluguéis e estadias",
  contactEyebrow: "Captação de leads",
  leadConsent: "Concordo que a Meirah Group entre em contato sobre minha solicitação.",
  adminEyebrow: "Modo interno",
  adminSelect: "Editar imóvel",
  newProperty: "Novo",
  ...
  adminStatus: "Status",
  adminPrice: "Preço de venda",
  adminNight: "Preço por noite",
  adminBeds: "Quartos",
  adminBaths: "Banheiros",
  adminZone: "Zona",
  adminDesc: "Descrição",
  saveProperty: "Salvar imóvel",
  resetDemo: "Restaurar demonstração",
  adminInsights: "Próximos passos recomendados",
  backupTitle: "Backup local",
  backupCopy: "Guarde uma cópia dos imóveis, reservas, reuniões, CRM e configurações deste navegador.",
  backupExport: "Exportar dados",
  backupImport: "Restaurar backup",
  missingOne: "Conecte um gateway local ou Stripe para cobranças reais e webhooks.",
  missingTwo: "Adicione etapas de CRM: novo, qualificado, visita, oferta e fechamento.",
  missingThree: "Envie documentos legais por imóvel: título, levantamento, impostos e autorizações.",
  missingFour: "Crie análises de leads, imóveis mais vistos e conversão de reservas.",
  missingFive: "Ative SEO técnico com URLs de imóveis e schema imobiliário.",
  footerCompany: "Empresa",
  footerServices: "Serviços",
  footerSocials: "Redes sociais",
  footerContact: "Contato",
  footerManagement: "Gestão de inventário",
  blogCta: "Peça um guia personalizado",
  blogOneMeta: "Investimento",
  blogOneTitle: "O que avaliar antes de comprar uma villa em uma área turística",
  blogTwoMeta: "Financiamento",
  blogTwoTitle: "Entrada, taxa e fluxo: como estimar sua parcela mensal",
  blogThreeMeta: "Jurídico",
  blogThreeTitle: "Documentos básicos para validar antes de reservar",
  ownersCopy:
    "Se você tem uma casa, villa, apartamento, terreno, fazenda ou espaço comercial, a Meirah Group pode representá-lo com estratégia de preço, fotos, divulgação, seleção de clientes, visitas, negociação e acompanhamento até o fechamento.",
  ownersSaleTitle: "Representação para venda",
  ownersSaleCopy: "Análise de preço, divulgação, clientes qualificados, visitas e negociação.",
  ownersRentTitle: "Gestão de aluguel",
  ownersRentCopy: "Divulgação para aluguéis de curto e longo prazo, reservas, disponibilidade e seleção de clientes.",
  ownersLandTitle: "Terrenos e fazendas",
  ownersLandCopy: "Apresentamos potencial de uso, acessos, serviços próximos e dados-chave de investimento.",
  ownersDocsTitle: "Preparação comercial",
  ownersDocsCopy: "Organizamos fotos, descrição, documentos básicos e argumentos para vender com confiança.",
  reviewsEyebrow: "Avaliações",
  reviewsTitle: "Referências verificadas de clientes",
  reviewOne: "A equipe nos ajudou a comparar regiões, negociar e entender os custos reais antes de assinar.",
  reviewTwo: "Reservamos uma villa para um fim de semana em família. Tudo foi rápido, claro e sem surpresas.",
  reviewThree: "Moro fora do país e precisava de alguém que respondesse com precisão. A Meirah Group facilitou tudo.",
  gallery: "Galeria",
  modalBooking: "Estimativa de estadia",
  modalPayment: "Plano de pagamentos",
  checkIn: "Entrada",
  checkOut: "Saída",
  guests: "Hóspedes",
  monthlyEstimate: "Parcela mensal estimada",
  financedAmount: "Valor financiado",
  totalInterest: "Juros estimados",
  currentPayment: "Parcela atual",
  newPayment: "Nova parcela",
  monthlySavings: "Economia mensal",
  breakEven: "Ponto de equilíbrio",
  maxHomePrice: "Preço máximo estimado",
  targetPayment: "Parcela recomendada",
  buyCost: "Custo estimado de compra",
  rentCost: "Custo estimado de aluguel",
  betterOption: "Opção sugerida",
...
function openAccessibleModal(modal, focusSelector = "button, [href], input, select, textarea") {
  if (!modal) return;
  if (document.activeElement instanceof HTMLElement) modalFocusOrigins.set(modal, document.activeElement);
  modal.hidden = false;
  document.body.classList.add("modal-open");
  requestAnimationFrame(() => $(focusSelector, modal)?.focus({ preventScroll: true }));
}
...
function closeAccessibleModal(modal) {
  ...
  if (origin && document.contains(origin)) requestAnimationFrame(() => origin.focus({ preventScroll: true }));
}
    ...
    return `
      ...
      <div>
        ...
        <p>${escapeHtml(localized({
          es: "Sube una imagen para verla antes de guardar.",
          en: "Upload an image to preview it before saving.",
          pt: "Envie uma imagem para visualizá-la antes de salvar.",
        }))}</p>
      </div>
    `;
  }
...
function upgradeDefaultStaffProfiles(staff) {
  return staff.map((member) => {
    ...
    if (!updated.name || /^diego cueto( hawa)?$/i.test(updated.name)) updated.name = "Diego Cueto";
    if (!updated.username || /^(diego|diego cueto|diego c\. hawa)$/i.test(updated.username)) updated.username = "diego.cueto";
...
function normalizeAccessRole(role, index = 0) {
  const permissions = Object.fromEntries(
    STAFF_PERMISSION_DEFINITIONS.map(({ key }) => [key, role?.permissions?.[key] === true])
  );
  return {
    id: String(role?.id || `role-${Date.now()}-${index}`).replace(/[^a-z0-9_-]/gi, "-").toLowerCase(),
    name: repairMojibakeText(String(role?.name || `Rol ${index + 1}`)).trim(),
    color: /^#[0-9a-f]{6}$/i.test(role?.color || "") ? role.color : "#1e2e47",
    permissions,
  };
}
...
function loadAccessRoles() {
  try {
    const saved = JSON.parse(localStorage.getItem(STORAGE.accessRoles) || "null");
    const roles = Array.isArray(saved) && saved.length ? saved : DEFAULT_ACCESS_ROLES;
    const normalized = roles.map(normalizeAccessRole);
    localStorage.setItem(STORAGE.accessRoles, JSON.stringify(normalized));
    return normalized;
  } catch {
    return structuredClone(DEFAULT_ACCESS_ROLES).map(normalizeAccessRole);
  }
}

function saveAccessRoles(roles = accessRoles) {
  accessRoles = roles.map(normalizeAccessRole);
  if (typeof state !== "undefined") state.accessRoles = accessRoles;
  localStorage.setItem(STORAGE.accessRoles, JSON.stringify(accessRoles));
}

function normalizeStaffCategory(category, index = 0) {
  return {
    id: String(category?.id || `category-${Date.now()}-${index}`).replace(/[^a-z0-9_-]/gi, "-").toLowerCase(),
    name: repairMojibakeText(String(category?.name || `Categoría ${index + 1}`)).trim(),
    color: /^#[0-9a-f]{6}$/i.test(category?.color || "") ? category.color : "#c89952",
  };
}

function loadStaffCategories() {
  try {
    const saved = JSON.parse(localStorage.getItem(STORAGE.staffCategories) || "null");
    const categories = Array.isArray(saved) && saved.length ? saved : DEFAULT_STAFF_CATEGORIES;
    const normalized = categories.map(normalizeStaffCategory);
    localStorage.setItem(STORAGE.staffCategories, JSON.stringify(normalized));
    return normalized;
  } catch {
    return structuredClone(DEFAULT_STAFF_CATEGORIES).map(normalizeStaffCategory);
  }
}

function saveStaffCategories(categories = staffCategories) {
  staffCategories = categories.map(normalizeStaffCategory);
  if (typeof state !== "undefined") state.staffCategories = staffCategories;
  localStorage.setItem(STORAGE.staffCategories, JSON.stringify(staffCategories));
}

function defaultStaffPermissions(role) {
  const definition = accessRoles.find((item) => item.id === role) || accessRoles[0] || DEFAULT_ACCESS_ROLES[0];
  return structuredClone(definition.permissions);
}
...
function normalizeStaffRole(role) {
  ...
  return accessRoles.some((item) => item.id === normalized) ? normalized : "advisor";
}
...
function normalizeStaffMember(member, index = 0) {
  ...
  return {
    ...
    role,
    categoryId: staffCategories.some((category) => category.id === member?.categoryId)
      ? member.categoryId
      : role === "ceo"
        ? "direction"
        : role === "developer"
    ...
    acceptsAppointments:
      typeof member?.acceptsAppointments === "boolean"
        ...
        : role === "advisor" && hasWeeklyAvailability,
...
function loadStaff() {
  try {
    ...
    const savedSeedVersion = localStorage.getItem("meirahgroup.staffSeedVersion");
    ...
    const source =
      savedSeedVersion === STAFF_SEED_VERSION && Array.isArray(saved) && saved.length && !isLegacyDefault
        ? saved
        : structuredClone(DEFAULT_STAFF);
    ...
    localStorage.setItem("meirahgroup.staffSeedVersion", STAFF_SEED_VERSION);
    return staff;
  } catch {
...
function loadCompanyProfile() {
  try {
    const saved = JSON.parse(localStorage.getItem(STORAGE.companyProfile) || "{}");
    return { ...DEFAULT_COMPANY_PROFILE, ...(saved && typeof saved === "object" ? saved : {}) };
  } catch {
...
function saveCompanyProfile(profile) {
  state.companyProfile = { ...DEFAULT_COMPANY_PROFILE, ...profile };
  localStorage.setItem(STORAGE.companyProfile, JSON.stringify(state.companyProfile));
}

function loadStaffSession(staff = []) {
  try {
    localStorage.removeItem(STORAGE.session);
    const session = JSON.parse(sessionStorage.getItem(STORAGE.session) || "null");
...
function saveStaffSession(member) {
  ...
  localStorage.removeItem(STORAGE.session);
  if (state.session) sessionStorage.setItem(STORAGE.session, JSON.stringify(state.session));
...
function hasStaffPermission(permission) {
  ...
  const role = state.accessRoles.find((item) => item.id === member.role);
  return role ? role.permissions?.[permission] === true : member.permissions?.[permission] === true;
}

function isManagementRole(member = currentStaffMember()) {
  if (!member) return false;
  const role = state.accessRoles.find((item) => item.id === member.role);
  return role?.permissions?.manageStaff === true;
}
...
function homePagePath() {
  ...
  if (pathname.toLowerCase().match(/\/(propiedades|calculadoras|contacto|admin)\.html$/i)) {
    return pathname.replace(/(propiedades|calculadoras|contacto|admin)\.html$/i, "");
  }
...
function adminPagePath() {
  if (window.location.protocol === "file:") return "admin.html";
  const base = propertyRouteBase();
  return `${base || ""}/admin.html`.replace(/\/{2,}/g, "/");
}
...
function openShareFallback(property) {
  ...
  `;
  openAccessibleModal(modal, "[data-close-share]");
...
function renderModalPropertySpecs(property) {
  ...
  const detailedSpecs = specs.map((spec, index) => {
    const shouldSpanFull = isTerrain && specs.length === 1;
    const classes = shouldSpanFull ? " property-spec-full" : "";
    return `
      <span class="${classes.trim()}">
        <strong>${escapeHtml(spec.value)}</strong>
        <small class="property-spec-icon" title="${escapeHtml(spec.label)}" aria-label="${escapeHtml(spec.label)}">
          <i data-lucide="${escapeHtml(spec.icon || "dot")}"></i>
        </small>
      </span>
    `;
  });
...
function propertySortArea(property) {
  return getPropertyTotalArea(property);
}

function propertyRecentValue(property) {
  const codeValue = Number.parseInt(String(property?.code || "").replace(/\D/g, ""), 10);
  if (Number.isFinite(codeValue) && codeValue > 0) return codeValue;
  const createdAtValue = Date.parse(property?.createdAt || "");
...
function applyDocumentLanguageMetadata() {
  const pageMode = document.body.dataset.pageMode || "home";
  const metadata = localized({
    es: {
      home: ["Meirah Group | Bienes Raíces en República Dominicana", "Meirah Group: propiedades, asesoría inmobiliaria y oportunidades de inversión en República Dominicana."],
      properties: ["Propiedades | Meirah Group", "Explora propiedades seleccionadas por Meirah Group para comprar, vender o alquilar."],
      calculators: ["Calculadoras financieras | Meirah Group", "Calculadoras de hipoteca, refinanciación, asequibilidad y alquiler versus compra."],
      contact: ["Contacto | Meirah Group", "Contacta a Meirah Group o agenda una cita con uno de nuestros agentes."],
    },
    en: {
      home: ["Meirah Group | Real Estate in the Dominican Republic", "Meirah Group: properties, real estate guidance, and investment opportunities in the Dominican Republic."],
      properties: ["Properties | Meirah Group", "Explore Meirah Group's curated properties to buy, sell, or rent."],
      calculators: ["Financial Calculators | Meirah Group", "Mortgage, refinance, affordability, and rent versus buy calculators."],
      contact: ["Contact | Meirah Group", "Contact Meirah Group or schedule a meeting with one of our agents."],
    },
    pt: {
      home: ["Meirah Group | Imóveis na República Dominicana", "Meirah Group: imóveis, assessoria imobiliária e oportunidades de investimento na República Dominicana."],
      properties: ["Imóveis | Meirah Group", "Explore imóveis selecionados pela Meirah Group para comprar, vender ou alugar."],
      calculators: ["Calculadoras financeiras | Meirah Group", "Calculadoras de hipoteca, refinanciamento, acessibilidade e alugar versus comprar."],
      contact: ["Contato | Meirah Group", "Entre em contato com a Meirah Group ou agende uma reunião com um de nossos agentes."],
    },
  });
  const [title, description] = metadata[pageMode] || metadata.home;
...
function applyAccessibilityTranslations() {
  const labels = localized({
    es: {
      home: "Inicio de Meirah Group", navigation: "Navegación principal", aboutMenu: "Opciones de nosotros",
      calculatorMenu: "Opciones de calculadoras", contactMenu: "Opciones de contacto", language: "Cambiar idioma",
      currency: "Cambiar moneda", admin: "Modo interno", indicators: "Indicadores", calculators: "Calculadoras",
      privacy: "Información de privacidad", previousMonth: "Mes anterior", nextMonth: "Mes siguiente",
      preferences: "Preferencias de notificación", removeAvatar: "Quitar avatar", openChat: "Abrir chatbot",
      chatbot: "Chatbot Meirah Group", closeChat: "Cerrar chat", send: "Enviar", close: "Cerrar",
      purpose: "Misión, visión y valores", values: "Valores de Meirah Group",
    },
    en: {
      home: "Meirah Group home", navigation: "Main navigation", aboutMenu: "About options",
      calculatorMenu: "Calculator options", contactMenu: "Contact options", language: "Change language",
      currency: "Change currency", admin: "Internal mode", indicators: "Key indicators", calculators: "Calculators",
      privacy: "Privacy information", previousMonth: "Previous month", nextMonth: "Next month",
      preferences: "Notification preferences", removeAvatar: "Remove avatar", openChat: "Open chatbot",
      chatbot: "Meirah Group chatbot", closeChat: "Close chat", send: "Send", close: "Close",
      purpose: "Mission, vision, and values", values: "Meirah Group values",
    },
    pt: {
      home: "Início da Meirah Group", navigation: "Navegação principal", aboutMenu: "Opções sobre nós",
      calculatorMenu: "Opções de calculadoras", contactMenu: "Opções de contato", language: "Alterar idioma",
      currency: "Alterar moeda", admin: "Modo interno", indicators: "Indicadores", calculators: "Calculadoras",
      privacy: "Informações de privacidade", previousMonth: "Mês anterior", nextMonth: "Próximo mês",
      preferences: "Preferências de notificação", removeAvatar: "Remover avatar", openChat: "Abrir chatbot",
      chatbot: "Chatbot da Meirah Group", closeChat: "Fechar chat", send: "Enviar", close: "Fechar",
      purpose: "Missão, visão e valores", values: "Valores da Meirah Group",
    },
  });
  const assignments = [
    [".brand", labels.home], [".main-nav", labels.navigation], [".nav-dropdown-menu[data-nav-dropdown='about']", labels.aboutMenu],
    [".nav-dropdown-menu[data-nav-dropdown='financing']", labels.calculatorMenu], [".nav-dropdown-menu[data-nav-dropdown='contact']", labels.contactMenu],
    ["#languageToggle", labels.language], ["#currencyToggle", labels.currency], ["#adminToggle", labels.admin],
    [".trust-band", labels.indicators], [".calculator-menu", labels.calculators], [".appointment-note-card", labels.privacy],
    ["#appointmentPrevMonth", labels.previousMonth], ["#appointmentNextMonth", labels.nextMonth], [".member-options", labels.preferences],
    ["#staffAvatarClear", labels.removeAvatar], ["#chatToggle", labels.openChat], ["#chatPanel", labels.chatbot],
    ["#chatClose", labels.closeChat], [".chat-form button[type='submit']", labels.send], ["[data-close-modal]", labels.close],
    ["[data-close-staff-login]", labels.close], ["[data-close-share]", labels.close], [".purpose-section", labels.purpose],
    [".purpose-values", labels.values],
  ];
  assignments.forEach(([selector, label]) => $$(selector).forEach((node) => node.setAttribute("aria-label", label)));
}
...
const DISCLOSURE_TRANSITION_MS = 180;

function setDisclosureState(menu, isOpen) {
  if (!menu) return;
  window.clearTimeout(menu._disclosureCloseTimer);

  if (isOpen) {
    menu.hidden = false;
    window.requestAnimationFrame(() => {
      if (!menu.hidden) menu.classList.add("is-revealed");
    });
    return;
  }

  menu.classList.remove("is-revealed");
  menu._disclosureCloseTimer = window.setTimeout(() => {
    if (!menu.classList.contains("is-revealed")) menu.hidden = true;
...
function closeSurfaceSelects(exceptId = "") {
  $$("[data-surface-select]").forEach((shell) => {
    ...
    setDisclosureState(menu, isOpen);
  });
...
function syncSurfaceSelect(selectOrId) {
  ...
  setDisclosureState(menu, openSurfaceSelect === select.id);
}
...
function toggleHeaderMenu(name) {
  ...
  setDisclosureState(menu, shouldOpen);
}
...
function applyPageMode() {
  ...
  const footerLinkMap = [
    [".site-footer .footer-brand", onHome ? "#top" : pageLink("home", "#top")],
    [".site-footer a[href=\"#about\"]", onHome ? "#about" : pageLink("home", "#about")],
    [".site-footer a[href=\"#properties\"]", onProperties ? "#properties" : pageLink("properties", "#properties")],
    [".site-footer a[href=\"#financing\"]", onCalculators ? "#financing" : pageLink("calculators", "#financing")],
    [".site-footer a[href=\"#reservations\"]", onProperties ? "#reservations" : pageLink("properties", "#reservations")],
    [".site-footer a[href=\"#owners\"]", onProperties ? "#owners" : pageLink("properties", "#owners")],
    [".site-footer a[href=\"#appointments\"]", onContact ? "#appointments" : pageLink("contact", "#appointments")],
    [".site-footer a[href=\"#contact\"]", onContact ? "#contact" : pageLink("contact", "#contact")],
    [".site-footer a[href=\"#adminPanel\"]", onAdmin ? "#adminPanel" : pageLink("admin", "#adminPanel")],
  ];

  footerLinkMap.forEach(([selector, href]) => {
...
function reorderPrimarySections() {
  ...
  if (pageMode === "contact") {
    ...
    if (members && appointments.nextElementSibling !== members) {
      appointments.insertAdjacentElement("afterend", members);
    }
    return;
  }
...
function applyTheme() {
  ...
  themeToggle.setAttribute(
    "aria-label",
    localized({
      es: state.theme === "dark" ? "Activar modo claro" : "Activar modo oscuro",
      en: state.theme === "dark" ? "Enable light mode" : "Enable dark mode",
      pt: state.theme === "dark" ? "Ativar modo claro" : "Ativar modo escuro",
    })
  );
  if (window.lucide) window.lucide.createIcons();
}
...
function getFilteredProperties() {
  ...
  properties = properties.slice().sort((a, b) => {
    ...
    if (sort === "area-asc") return propertySortArea(a) - propertySortArea(b);
    if (sort === "area-desc") return propertySortArea(b) - propertySortArea(a);
    if (sort === "recent") return propertyRecentValue(b) - propertyRecentValue(a);
  ...
  const rentButton = shouldShowReservation(property)
    ? `<button class="button button-light" type="button" data-open-property="${property.id}" data-focus-booking="true">
      ...
      : `<a class="button button-light" href="https://wa.me/${BUSINESS_WHATSAPP}?text=${encodeURIComponent(
        localized({
          es: `Hola ${BUSINESS_NAME}, quiero consultar ${getTitle(property)}.`,
          en: `Hello ${BUSINESS_NAME}, I would like to inquire about ${getTitle(property)}.`,
          pt: `Olá ${BUSINESS_NAME}, gostaria de consultar ${getTitle(property)}.`,
        })
      ...
      <div class="property-media" data-property-gallery="${property.id}" data-image-index="0" data-active-slot="0">
        ...
        <div class="badge-row">
          ...
          <button class="favorite-button ${isFavorite ? "active" : ""}" type="button" data-favorite="${
            property.id
          }" aria-label="${escapeHtml(text("favoritesButton"))}">
...
function openPropertyModal(propertyId, focusBooking = false, updateHistory = false) {
  ...
  $("#modalBody").innerHTML = `
    <div class="modal-grid">
      ...
      <div class="modal-details">
        ...
        <p class="property-zone">${escapeHtml(property.zone)} · ${escapeHtml(typeLabel(property.type))} · ${text("propertyCode")} ${escapeHtml(property.code)}</p>
        <p class="property-price"><strong>${price}</strong></p>
        <div class="property-specs property-specs-detailed modal-property-specs">
          ${renderModalPropertySpecs(property)}
        </div>
        <p>${escapeHtml(getDescription(property))}</p>
        <div class="modal-tools">
          ...
          <a class="button button-primary" href="https://wa.me/${BUSINESS_WHATSAPP}?text=${encodeURIComponent(
            localized({
              es: `Hola ${BUSINESS_NAME}, quiero información sobre ${getTitle(property)}.`,
              en: `Hello ${BUSINESS_NAME}, I would like information about ${getTitle(property)}.`,
              pt: `Olá ${BUSINESS_NAME}, gostaria de obter informações sobre ${getTitle(property)}.`,
            })
  ...
  openAccessibleModal($("#propertyModal"), "[data-close-modal]");
...
function closeModal(updateHistory = true) {
  ...
  closeAccessibleModal($("#propertyModal"));
...
function handleBookingSubmit(form) {
  ...
  $("[data-booking-output]", form).innerHTML = `
    <div class="schedule-row"><span>${formatNumber(nights)} ${text("nights")} x ${formatCurrency(
    property.rentNight
  )}</span><strong>${formatCurrency(subtotal)}</strong></div>
    <div class="schedule-row"><span>${text("cleaning")}</span><strong>${formatCurrency(
    cleaning
  )}</strong></div>
    <div class="schedule-row"><span>${text("total")}</span><strong>${formatCurrency(total)}</strong></div>
    <a class="button button-primary" href="https://wa.me/${BUSINESS_WHATSAPP}?text=${encodeURIComponent(
      localized({
        es: `Hola ${BUSINESS_NAME}, quiero reservar ${getTitle(property)} del ${data.get("checkIn")} al ${data.get("checkOut")}.`,
        en: `Hello ${BUSINESS_NAME}, I would like to book ${getTitle(property)} from ${data.get("checkIn")} to ${data.get("checkOut")}.`,
        pt: `Olá ${BUSINESS_NAME}, gostaria de reservar ${getTitle(property)} de ${data.get("checkIn")} a ${data.get("checkOut")}.`,
      })
...
const LOCAL_BACKUP_KEYS = [...new Set([...Object.values(STORAGE), "meirahgroup.staffSeedVersion"])];

function createLocalBackup() {
  const data = Object.fromEntries(
    LOCAL_BACKUP_KEYS
      .filter((key) => key !== STORAGE.session)
      .map((key) => [key, localStorage.getItem(key)])
      .filter(([, value]) => value !== null)
  );
  return {
    schema: "meirahgroup.local-backup",
    version: 1,
...
function downloadLocalBackup() {
  const backup = JSON.stringify(createLocalBackup(), null, 2);
  const blob = new Blob([backup], { type: "application/json" });
  const url = URL.createObjectURL(blob);
  const anchor = document.createElement("a");
  anchor.href = url;
  anchor.download = `meirahgroup-respaldo-${formatDateInput(new Date())}.json`;
  document.body.appendChild(anchor);
  anchor.click();
  anchor.remove();
  URL.revokeObjectURL(url);
  showToast(localized({ es: "Respaldo descargado.", en: "Backup downloaded.", pt: "Backup baixado." }));
}

function restoreLocalBackup(file) {
  if (!file) return;
  if (file.size > 5 * 1024 * 1024) {
    showToast(localized({ es: "El respaldo supera el límite de 5 MB.", en: "The backup exceeds the 5 MB limit.", pt: "O backup excede o limite de 5 MB." }));
    return;
  }
  const reader = new FileReader();
  reader.onload = () => {
    try {
      ...
      if (backup?.schema !== "meirahgroup.local-backup" || !backup.data || typeof backup.data !== "object") {
        throw new Error("invalid-backup");
      }
      const entries = Object.entries(backup.data).filter(
        ([key, value]) => LOCAL_BACKUP_KEYS.includes(key) && typeof value === "string"
      );
      if (!entries.length) throw new Error("empty-backup");
      const accepted = window.confirm(
        localized({
          es: "Restaurar este respaldo reemplazará los datos locales actuales de Meirah Group en este navegador. ¿Continuar?",
          en: "Restoring this backup will replace the current Meirah Group local data in this browser. Continue?",
          pt: "Restaurar este backup substituirá os dados locais atuais da Meirah Group neste navegador. Continuar?",
        })
      );
      if (!accepted) return;
      LOCAL_BACKUP_KEYS.forEach((key) => localStorage.removeItem(key));
      entries.forEach(([key, value]) => localStorage.setItem(key, value));
      sessionStorage.removeItem(STORAGE.session);
      window.location.reload();
    } catch {
      showToast(localized({ es: "No pudimos leer ese respaldo.", en: "We could not read that backup.", pt: "Não foi possível ler esse backup." }));
    }
  };
...
function populateCompanyProfileForm() {
  ...
  const fields = {
    ...
    companyProfileAddress: "address",
    companyProfileCoverage: "coverage",
    companyProfileHours: "hours",
    companyProfileLanguages: "languages",
    companyProfileDescription: "description",
    companyProfileServices: "services",
    companyProfileReservationPolicy: "reservationPolicy",
    companyProfileAppointmentPolicy: "appointmentPolicy",
    companyProfileFaqs: "faqs",
    companyProfileAdditional: "additionalInfo",
  };
...
function collectCompanyProfileForm() {
  const fields = {
    ...
    hours: "companyProfileHours",
    languages: "companyProfileLanguages",
    description: "companyProfileDescription",
    services: "companyProfileServices",
    reservationPolicy: "companyProfileReservationPolicy",
    appointmentPolicy: "companyProfileAppointmentPolicy",
    faqs: "companyProfileFaqs",
    additionalInfo: "companyProfileAdditional",
  };
  return Object.fromEntries(
    Object.entries(fields).map(([key, id]) => [key, $(`#${id}`)?.value.trim() || ""])
  );
}

function setupCompanyProfileAdmin() {
  const section = $("#companyProfileSection");
  if (!section) return;
  section.hidden = !hasStaffPermission("manageStaff");
  if (!section.hidden) populateCompanyProfileForm();
}
...
function collectAdminProperty(existing) {
  ...
  return {
    ...
    description:
      $("#adminDescInput").value.trim() ||
      localized({
        es: "Propiedad agregada al inventario de Meirah Group.",
        en: "Property added to the Meirah Group inventory.",
        pt: "Imóvel adicionado ao inventário da Meirah Group.",
      }),
...
function parseCompanyFaqs() {
  return String(state.companyProfile?.faqs || "")
    .split(/\r?\n/)
    .map((line) => {
...
function getCompanyProfileAnswer(question) {
  const q = normalizeSearchText(question);
  const profile = state.companyProfile || DEFAULT_COMPANY_PROFILE;
  const hasTerm = (...terms) => terms.some((term) => q.includes(term));
  const value = (key) => String(profile[key] || "").trim();

  if (hasTerm("horario", "horarios", "hours", "abierto", "open")) {
    return localized({ es: `Horario: ${value("hours")}`, en: `Business hours: ${value("hours")}`, pt: `Horário: ${value("hours")}` });
  }
  if (hasTerm("ubic", "where", "location", "direccion", "address", "jarabacoa")) {
    return localized({
      es: `Ubicación: ${value("address")}. Cobertura: ${value("coverage")}`,
      en: `Location: ${value("address")}. Coverage: ${value("coverage")}`,
      pt: `Localização: ${value("address")}. Cobertura: ${value("coverage")}`,
    });
  }
  if (hasTerm("servicio", "services", "ofrecen", "offer")) {
    return localized({ es: `Servicios: ${value("services")}`, en: `Services: ${value("services")}`, pt: `Serviços: ${value("services")}` });
  }
  if (hasTerm("zona", "zonas", "areas", "coverage", "cobertura", "trabajan", "work")) {
    return localized({ es: `Trabajamos en ${value("coverage")}`, en: `We serve ${value("coverage")}`, pt: `Atendemos ${value("coverage")}` });
  }
  if (hasTerm("idioma", "language", "portugues", "english", "ingles", "espanol")) {
    return localized({ es: `Idiomas disponibles: ${value("languages")}`, en: `Available languages: ${value("languages")}`, pt: `Idiomas disponíveis: ${value("languages")}` });
  }
  if (hasTerm("reserva", "booking", "airbnb")) {
    return localized({ es: value("reservationPolicy"), en: value("reservationPolicy"), pt: value("reservationPolicy") });
  }
  if (hasTerm("cita", "appointment", "reunion", "reuniao")) {
    return localized({ es: value("appointmentPolicy"), en: value("appointmentPolicy"), pt: value("appointmentPolicy") });
  }
  if (hasTerm("quienes", "quien es", "empresa", "about", "sobre", "meirah")) {
    return localized({ es: value("description"), en: value("description"), pt: value("description") });
  }
  if (hasTerm("correo", "email", "whatsapp", "telefono", "phone", "contato", "contact")) {
    return localized({
      es: `Puedes escribirnos al ${value("phone")} o a ${value("email")}.`,
      en: `You can contact us at ${value("phone")} or ${value("email")}.`,
      pt: `Você pode falar conosco pelo ${value("phone")} ou ${value("email")}.`,
    });
  }

  const questionWords = q.split(/[^a-z0-9]+/).filter((word) => word.length > 3);
  const faq = parseCompanyFaqs().find((item) =>
    questionWords.some((word) => normalizeSearchText(item.question).includes(word))
  );
  if (faq?.answer) return faq.answer;

  const additionalInfo = value("additionalInfo");
  return questionWords.some((word) => normalizeSearchText(additionalInfo).includes(word)) ? additionalInfo : "";
}
...
function getBotAnswer(question) {
  ...
  const profileAnswer = getCompanyProfileAnswer(question);
...
function closeAppointmentSelectors() {
  ...
  $$("[data-appointment-selector-card]").forEach((card) => {
    ...
    setDisclosureState(panel, false);
    if (toggle) toggle.setAttribute("aria-expanded", "false");
  });
}
...
function toggleAppointmentSelector(key) {
  ...
  $$("[data-appointment-selector-card]").forEach((card) => {
    ...
    setDisclosureState(panel, active);
...
function staffRoleLabel(role) {
  return state.accessRoles.find((item) => item.id === role)?.name || role;
}
...
function staffListForSession() {
  ...
  return hasStaffPermission("ownAvailability")
    ? state.staff.filter((member) => member.id === current.id)
    : [];
}

function renderStaffAssignmentOptions(member) {
  const roleSelect = $("#staffRoleInput");
  const categorySelect = $("#staffCategoryInput");
  if (roleSelect) {
    roleSelect.innerHTML = state.accessRoles
      .map((role) => `<option value="${escapeHtml(role.id)}">${escapeHtml(role.name)}</option>`)
      .join("");
    roleSelect.value = member?.role || state.accessRoles[0]?.id || "";
  }
  if (categorySelect) {
    categorySelect.innerHTML = state.staffCategories
      .map((category) => `<option value="${escapeHtml(category.id)}">${escapeHtml(category.name)}</option>`)
      .join("");
    categorySelect.value = member?.categoryId || state.staffCategories[0]?.id || "";
  }
}

function renderAccessPermissionGrid(role) {
  const container = $("#accessPermissionGrid");
  if (!container || !role) return;
  container.innerHTML = STAFF_PERMISSION_DEFINITIONS.map(
    ({ key, label }) => `
      <label class="permission-chip${role.permissions[key] ? " is-enabled" : ""}">
        <input type="checkbox" data-role-permission="${key}" ${role.permissions[key] ? "checked" : ""} />
        <span><b class="permission-symbol" aria-hidden="true">${role.permissions[key] ? "✓" : "×"}</b>${escapeHtml(label)}</span>
      </label>
    `
  ).join("");
...
function renderAccessManagement(selectedRoleId = $("#accessRoleSelect")?.value) {
  const canManage = hasStaffPermission("manageStaff");
  const managementGrid = document.querySelector(".access-management-grid");
  if (managementGrid) managementGrid.hidden = !canManage;
  if ($("#accessUserCount")) $("#accessUserCount").textContent = String(state.staff.length);
  if ($("#accessRoleCount")) $("#accessRoleCount").textContent = String(state.accessRoles.length);
  if ($("#accessCategoryCount")) $("#accessCategoryCount").textContent = String(state.staffCategories.length);

  renderStaffAssignmentOptions(state.staff.find((member) => member.id === $("#staffIdInput")?.value));
  if (!canManage) return;

  const roleSelect = $("#accessRoleSelect");
  const role = state.accessRoles.find((item) => item.id === selectedRoleId) || state.accessRoles[0];
  roleSelect.innerHTML = state.accessRoles.map((item) => `<option value="${escapeHtml(item.id)}">${escapeHtml(item.name)}</option>`).join("");
  if (role) {
    roleSelect.value = role.id;
    $("#accessRoleName").value = role.name;
    $("#accessRoleColor").value = role.color;
    renderAccessPermissionGrid(role);
  }

  const categoryList = $("#accessCategoryList");
  categoryList.innerHTML = state.staffCategories.map((category) => {
    const total = state.staff.filter((member) => member.categoryId === category.id).length;
    return `<div class="category-row"><span class="category-dot" style="--category-color:${escapeHtml(category.color)}"></span><strong>${escapeHtml(category.name)}</strong><small>${total}</small><button type="button" data-delete-category="${escapeHtml(category.id)}" aria-label="Eliminar ${escapeHtml(category.name)}"><i data-lucide="x"></i></button></div>`;
  }).join("");
  window.lucide?.createIcons();
}
...
function renderStaffAdminList(selectedId = $("#staffIdInput")?.value) {
  ...
  container.innerHTML = staffListForSession()
    .map((member) => {
      ...
      const role = state.accessRoles.find((item) => item.id === member.role);
      const category = state.staffCategories.find((item) => item.id === member.categoryId);
      return `
        <button class="staff-admin-list-item${member.id === selectedId ? " is-selected" : ""}" type="button" data-edit-staff="${member.id}">
          ...
          <span><strong>${escapeHtml(member.name)}</strong><small><b class="role-dot" style="--role-color:${escapeHtml(role?.color || "#1e2e47")}"></b>${escapeHtml(staffRoleLabel(member.role))}${category ? ` · ${escapeHtml(category.name)}` : ""}</small></span>
        </button>
      `;
    })
    .join("");
...
function fillStaffAdminForm(member) {
  if (!member) return;
  renderStaffAssignmentOptions(member);
  ...
  $("#staffCategoryInput").value = member.categoryId;
  ...
  const agentOnly = !hasStaffPermission("manageStaff");
...
function newStaffDraft() {
  return normalizeStaffMember({
    ...
    username: "",
    password: "demo123",
    role: state.accessRoles[0]?.id || "advisor",
    categoryId: state.staffCategories[0]?.id || "commercial",
    permissions: defaultStaffPermissions(state.accessRoles[0]?.id || "advisor"),
    active: true,
    acceptsAppointments: false,
    availabilityConfigured: false,
    phone: BUSINESS_WHATSAPP,
...
function collectStaffFromForm(existing) {
  ...
  const fullName = $("#staffNameInput").value.trim().replace(/\s+/g, " ");
  ...
  return normalizeStaffMember({
    ...
    name: fullName,
    username: generatedUsername || $("#staffUsernameInput").value.trim(),
    password: $("#staffPasswordInput").value,
    role,
    categoryId: $("#staffCategoryInput").value,
...
function appointmentVisibleToSession(appointment) {
  ...
  return hasStaffPermission("manageAppointments") || appointment.staffId === current.id;
}
...
function populateAppointmentAdminStaffFilter() {
  ...
  const ownOnly = !hasStaffPermission("manageAppointments");
...
function normalizeCrmLead(lead, index = 0) {
  const createdAt = lead?.createdAt || new Date().toISOString();
  return {
    id: String(lead?.id || `lead-${Date.now()}-${index}`),
    name: repairMojibakeText(String(lead?.name || "")).trim(),
    phone: String(lead?.phone || "").trim(),
    email: String(lead?.email || "").trim(),
    interest: repairMojibakeText(String(lead?.interest || "")).trim(),
    message: repairMojibakeText(String(lead?.message || "")).trim(),
    stage: CRM_STAGES.some((stage) => stage.id === lead?.stage) ? lead.stage : "new",
    priority: CRM_PRIORITIES.includes(lead?.priority) ? lead.priority : "medium",
    assigneeId: String(lead?.assigneeId || ""),
    notes: repairMojibakeText(String(lead?.notes || "")).trim(),
    followUpDate: /^\d{4}-\d{2}-\d{2}$/.test(lead?.followUpDate || "") ? lead.followUpDate : "",
    source: String(lead?.source || "Formulario web"),
    createdAt,
...
function loadLeads() {
  try {
    ...
    const normalized = Array.isArray(leads) ? leads.map(normalizeCrmLead) : [];
    localStorage.setItem(STORAGE.leads, JSON.stringify(normalized));
    return normalized;
  } catch {
    return [];
  }
...
function saveLeads(leads) {
  const normalized = leads.map(normalizeCrmLead);
  localStorage.setItem(STORAGE.leads, JSON.stringify(normalized));
  return normalized;
}

function crmStageLabel(stageId) {
  return CRM_STAGES.find((stage) => stage.id === stageId)?.label || stageId;
}
...
function crmAssigneeName(lead) {
  return state.staff.find((member) => member.id === lead.assigneeId)?.name || "Sin asignar";
}

function crmFilteredLeads() {
  const query = $("#crmSearch")?.value.trim().toLocaleLowerCase("es") || "";
  const assignee = $("#crmAssigneeFilter")?.value || "all";
  const priority = $("#crmPriorityFilter")?.value || "all";
  return loadLeads().filter((lead) => {
    const haystack = `${lead.name} ${lead.phone} ${lead.interest}`.toLocaleLowerCase("es");
...
function populateCrmStaffOptions() {
  const activeStaff = state.staff.filter((member) => member.active);
  const filter = $("#crmAssigneeFilter");
  const editor = $("#crmLeadAssignee");
  if (filter) {
    const current = filter.value || "all";
    filter.innerHTML = `<option value="all">Todos</option><option value="">Sin asignar</option>${activeStaff
  ...
  if (editor) {
    editor.innerHTML = `<option value="">Sin asignar</option>${activeStaff
      .map((member) => `<option value="${escapeHtml(member.id)}">${escapeHtml(member.name)}</option>`)
      .join("")}`;
  }
}

function renderCrmSummary(leads) {
  const summary = $("#crmSummary");
  if (!summary) return;
  const pending = leads.filter((lead) => lead.followUpDate && lead.followUpDate <= formatDateInput(new Date()) && lead.stage !== "closed").length;
  const closed = leads.filter((lead) => lead.stage === "closed").length;
  summary.innerHTML = `
    <div><strong>${leads.length}</strong><span>Total</span></div>
    <div><strong>${leads.filter((lead) => lead.stage === "new").length}</strong><span>Nuevos</span></div>
    <div><strong>${pending}</strong><span>Seguimientos</span></div>
    <div><strong>${closed}</strong><span>Cierres</span></div>
  `;
}
...
function renderCrmBoard() {
  ...
  if (!board) return;
  const leads = crmFilteredLeads().sort((a, b) => new Date(b.updatedAt) - new Date(a.updatedAt));
  renderCrmSummary(loadLeads());
  board.innerHTML = CRM_STAGES.map((stage) => {
    const stageLeads = leads.filter((lead) => lead.stage === stage.id);
    return `
      <section class="crm-column" data-crm-stage="${stage.id}">
        <header><span><i data-lucide="${stage.icon}"></i>${stage.label}</span><b>${stageLeads.length}</b></header>
        <div class="crm-column-list">
          ${stageLeads.length ? stageLeads.map((lead) => `
            <button class="crm-card priority-${lead.priority}" type="button" data-edit-crm-lead="${escapeHtml(lead.id)}">
              <span class="crm-card-top"><strong>${escapeHtml(lead.name || "Sin nombre")}</strong><small>${crmPriorityLabel(lead.priority)}</small></span>
              <span>${escapeHtml(lead.interest || "Sin interés definido")}</span>
              <span class="crm-card-meta"><i data-lucide="user-round"></i>${escapeHtml(crmAssigneeName(lead))}</span>
              ${lead.followUpDate ? `<time class="${lead.followUpDate <= formatDateInput(new Date()) && lead.stage !== "closed" ? "is-due" : ""}"><i data-lucide="clock-3"></i>${escapeHtml(formatAppointmentDate(lead.followUpDate))}</time>` : ""}
            </button>
          `).join("") : `<p class="crm-column-empty">Sin contactos</p>`}
        </div>
      </section>
    `;
  }).join("");
...
function openCrmEditor(lead) {
  const editor = $("#crmEditor");
  if (!editor) return;
  const isNew = !lead?.id;
  const draft = normalizeCrmLead(lead || {
    id: `lead-${Date.now()}-${Math.random().toString(36).slice(2, 7)}`,
    stage: "new",
    priority: "medium",
    assigneeId: currentStaffMember()?.id || "",
  });
  populateCrmStaffOptions();
  $("#crmEditorTitle").textContent = isNew ? "Nuevo prospecto" : draft.name || "Editar contacto";
  $("#crmLeadId").value = isNew ? "" : draft.id;
  $("#crmLeadName").value = draft.name;
  $("#crmLeadPhone").value = draft.phone;
  $("#crmLeadInterest").value = draft.interest;
  $("#crmLeadStage").innerHTML = CRM_STAGES.map((stage) => `<option value="${stage.id}">${stage.label}</option>`).join("");
  $("#crmLeadStage").value = draft.stage;
  $("#crmLeadAssignee").value = draft.assigneeId;
  $("#crmLeadPriority").value = draft.priority;
  $("#crmLeadFollowUp").value = draft.followUpDate;
  $("#crmLeadNotes").value = draft.notes || draft.message;
  $("#crmDeleteLead").hidden = isNew;
  const digits = draft.phone.replace(/\D/g, "");
  $("#crmWhatsApp").hidden = !digits;
  $("#crmWhatsApp").href = digits ? `https://wa.me/${digits}?text=${encodeURIComponent(`Hola ${draft.name || ""}, te contactamos de Meirah Group.`)}` : "#";
  editor.hidden = false;
  $("#crmLeadName").focus();
  window.lucide?.createIcons();
}

function collectCrmLead(existing) {
  const now = new Date().toISOString();
  return normalizeCrmLead({
    ...existing,
    id: existing?.id || `lead-${Date.now()}-${Math.random().toString(36).slice(2, 7)}`,
    name: $("#crmLeadName").value,
    phone: $("#crmLeadPhone").value,
    interest: $("#crmLeadInterest").value,
    stage: $("#crmLeadStage").value,
    assigneeId: $("#crmLeadAssignee").value,
    priority: $("#crmLeadPriority").value,
    followUpDate: $("#crmLeadFollowUp").value,
    notes: $("#crmLeadNotes").value,
    createdAt: existing?.createdAt || now,
    updatedAt: now,
    source: existing?.source || "Registro manual",
  });
}

function setupCrmAdmin() {
  const section = $("#crmAdminSection");
  if (!section || !currentStaffMember()) return;
  section.hidden = !hasStaffPermission("viewLeads");
  if (section.hidden) return;
  populateCrmStaffOptions();
  renderCrmBoard();
}
...
function appointmentIsArchived(appointment) {
  const appointmentDate = parseDateOnly(appointment?.date);
  if (!appointmentDate) return false;
  const cutoff = new Date();
  cutoff.setHours(0, 0, 0, 0);
  cutoff.setDate(cutoff.getDate() - 7);
  return appointmentDate < cutoff;
}
...
function renderAppointmentAdmin() {
  ...
  const showHistory = $("#appointmentHistoryToggle")?.getAttribute("aria-pressed") === "true";
  const visibleAppointments = loadAppointments().filter(appointmentVisibleToSession);
  const archivedCount = visibleAppointments.filter(appointmentIsArchived).length;
  const historyButton = $("#appointmentHistoryToggle");
  const historyLabel = $("#appointmentHistoryToggleLabel");
  if (historyButton) {
    historyButton.disabled = archivedCount === 0 && !showHistory;
    historyButton.classList.toggle("is-active", showHistory);
  }
  if (historyLabel) historyLabel.textContent = showHistory ? "Volver a recientes" : `Historial · ${archivedCount}`;

  const appointments = visibleAppointments
    .filter((appointment) => showHistory ? appointmentIsArchived(appointment) : !appointmentIsArchived(appointment))
    ...
    .sort((a, b) => showHistory
      ? `${b.date}T${b.time}`.localeCompare(`${a.date}T${a.time}`)
      : `${a.date}T${a.time}`.localeCompare(`${b.date}T${b.time}`));

  if (!appointments.length) {
    container.innerHTML = `<p class="admin-empty-state">${showHistory ? "No hay citas archivadas con estos filtros." : localized({ es: "No hay citas recientes con estos filtros.", en: "No recent appointments match these filters.", pt: "Não há reuniões recentes com estes filtros." })}</p>`;
    return;
  }
...
function openStaffLogin() {
  ...
  const profiles = $("#staffLoginProfiles");
  if (profiles) {
    profiles.innerHTML = state.staff
      .filter((member) => member.active)
      .map((member) => `<option value="${escapeHtml(member.name)}"></option>`)
      .join("");
  }
  openAccessibleModal($("#staffLoginModal"), "#staffLoginUsername");
}

function closeStaffLogin() {
  closeAccessibleModal($("#staffLoginModal"));
...
function applyAdminPermissions() {
  ...
  $("#staffAdminSection").hidden = true;
  const crmSection = $("#crmAdminSection");
  if (crmSection) crmSection.hidden = true;
  $("#appointmentAdminSection").hidden = true;
  const companyProfileSection = $("#companyProfileSection");
  if (companyProfileSection) companyProfileSection.hidden = true;
...
function bindEvents() {
  ...
  $("#companyProfileForm")?.addEventListener("submit", (event) => {
    ...
    saveCompanyProfile(collectCompanyProfileForm());
    showToast(localized({
      es: "Información pública actualizada.",
      en: "Public information updated.",
      pt: "Informações públicas atualizadas.",
    }));
  });
  ...
  $("#leadForm").addEventListener("submit", (event) => {
    ...
    if (currentStaffMember()) renderCrmBoard();
    showToast(
      localized({
  ...
  $("#adminToggle").addEventListener("click", () => {
    if (detectPageMode() !== "admin") {
      window.location.href = pageLink("admin");
      return;
    }
  ...
  $("#staffLoginForm").addEventListener("submit", (event) => {
    event.preventDefault();
    const profileName = $("#staffLoginUsername").value.trim().replace(/\s+/g, " ");
    const password = $("#staffLoginPassword").value;
    const member = state.staff.find(
      (item) => item.active && item.name.localeCompare(profileName, "es", { sensitivity: "accent" }) === 0 && item.password === password
    );
    if (!member) {
      $("#staffLoginError").textContent = localized({
        es: "Nombre completo o contraseña incorrectos. Usa el nombre tal como aparece en el perfil.",
        en: "Incorrect full name or password. Use the name exactly as shown in the profile.",
        pt: "Nome completo ou senha incorretos. Use o nome exatamente como aparece no perfil.",
      });
      return;
    }
  ...
  $("#staffLogout").addEventListener("click", () => {
    ...
    setupStaffLogin();
    if (detectPageMode() === "admin") openStaffLogin();
    showToast(localized({ es: "Sesión cerrada.", en: "Signed out.", pt: "Sessão encerrada." }));
  });

  $("#accessRoleSelect")?.addEventListener("change", (event) => renderAccessManagement(event.target.value));

  $("#accessPermissionGrid")?.addEventListener("change", (event) => {
    const input = event.target.closest("[data-role-permission]");
    if (!input) return;
    const chip = input.closest(".permission-chip");
    chip?.classList.toggle("is-enabled", input.checked);
    const symbol = chip?.querySelector(".permission-symbol");
    if (symbol) symbol.textContent = input.checked ? "✓" : "×";
  });

  $("#newAccessRole")?.addEventListener("click", () => {
    if (!hasStaffPermission("manageStaff")) return;
    ...
    saveAccessRoles([...state.accessRoles, role]);
    renderAccessManagement(role.id);
    $("#accessRoleName")?.select();
  });

  $("#saveAccessRole")?.addEventListener("click", () => {
    if (!hasStaffPermission("manageStaff")) return;
    const id = $("#accessRoleSelect").value;
    const name = $("#accessRoleName").value.trim();
    if (!name) return;
    const permissions = Object.fromEntries(
      STAFF_PERMISSION_DEFINITIONS.map(({ key }) => [key, Boolean(document.querySelector(`[data-role-permission="${key}"]`)?.checked)])
    );
    const roles = state.accessRoles.map((role) => role.id === id
      ? { ...role, name, color: $("#accessRoleColor").value, permissions }
      : role);
    saveAccessRoles(roles);
    state.staff = state.staff.map((member) => member.role === id ? { ...member, permissions: structuredClone(permissions) } : member);
    saveStaff();
    renderAccessManagement(id);
    renderStaffAdminList();
    applyAdminPermissions();
    setupStaffAdmin($("#staffIdInput").value);
    setupAppointmentAdmin();
    setupCompanyProfileAdmin();
    showToast("Rol guardado. Los accesos del equipo ya están actualizados.");
  });

  $("#deleteAccessRole")?.addEventListener("click", () => {
    if (!hasStaffPermission("manageStaff") || state.accessRoles.length <= 1) return;
    const id = $("#accessRoleSelect").value;
    const assigned = state.staff.filter((member) => member.role === id).length;
    if (assigned) {
      showToast(`No puedes eliminarlo: está asignado a ${assigned} perfil${assigned === 1 ? "" : "es"}.`);
      return;
    }
    saveAccessRoles(state.accessRoles.filter((role) => role.id !== id));
    renderAccessManagement();
  });

  $("#categoryCreatorForm")?.addEventListener("submit", (event) => {
    event.preventDefault();
    if (!hasStaffPermission("manageStaff")) return;
    const name = $("#categoryNameInput").value.trim();
    if (!name) return;
    const category = normalizeStaffCategory({ id: `category-${Date.now()}`, name, color: $("#categoryColorInput").value });
    saveStaffCategories([...state.staffCategories, category]);
    event.currentTarget.reset();
    $("#categoryColorInput").value = "#c89952";
    renderAccessManagement();
  });

  $("#accessCategoryList")?.addEventListener("click", (event) => {
    const button = event.target.closest("[data-delete-category]");
    if (!button || !hasStaffPermission("manageStaff") || state.staffCategories.length <= 1) return;
    const id = button.dataset.deleteCategory;
    const assigned = state.staff.filter((member) => member.categoryId === id).length;
    if (assigned) {
      showToast(`Reasigna primero los ${assigned} perfiles de esta categoría.`);
      return;
    }
    saveStaffCategories(state.staffCategories.filter((category) => category.id !== id));
    renderAccessManagement();
  });
  ...
  $("#crmNewLead")?.addEventListener("click", () => openCrmEditor());

  $("#crmCloseEditor")?.addEventListener("click", () => {
    $("#crmEditor").hidden = true;
  });

  $("#crmBoard")?.addEventListener("click", (event) => {
    const card = event.target.closest("[data-edit-crm-lead]");
    if (!card) return;
    const lead = loadLeads().find((item) => item.id === card.dataset.editCrmLead);
    if (lead) openCrmEditor(lead);
  });

  $("#crmSearch")?.addEventListener("input", renderCrmBoard);
  $("#crmAssigneeFilter")?.addEventListener("change", renderCrmBoard);
  $("#crmPriorityFilter")?.addEventListener("change", renderCrmBoard);

  $("#crmEditor")?.addEventListener("submit", (event) => {
    event.preventDefault();
    if (!hasStaffPermission("viewLeads")) return;
    const id = $("#crmLeadId").value;
    const leads = loadLeads();
    const existing = leads.find((lead) => lead.id === id);
    const nextLead = collectCrmLead(existing);
    saveLeads(existing
      ? leads.map((lead) => lead.id === existing.id ? nextLead : lead)
      : [...leads, nextLead]);
    $("#crmEditor").hidden = true;
    renderCrmBoard();
    showToast(existing ? "Prospecto actualizado." : "Prospecto agregado al CRM.");
  });

  $("#crmDeleteLead")?.addEventListener("click", () => {
    const id = $("#crmLeadId").value;
    if (!id || !hasStaffPermission("viewLeads")) return;
    if (!window.confirm("¿Eliminar este prospecto del CRM?")) return;
  ...
  $("#appointmentHistoryToggle")?.addEventListener("click", (event) => {
  ...
  $("#importSiteDataTrigger")?.addEventListener("click", () => {
    if (!hasStaffPermission("manageProperties")) return;
    $("#importSiteData")?.click();
  });

  $("#importSiteData")?.addEventListener("change", (event) => {
    if (!hasStaffPermission("manageProperties")) return;
    restoreLocalBackup(event.target.files?.[0]);
    event.target.value = "";
  });
...
function init() {
  ...
  setupAdmin();
  if (document.body.dataset.pageMode === "admin") {
    if (currentStaffMember()) {
      $("#adminPanel").hidden = false;
      setupAdmin();
    } else {
